...
//second camera
image_transport::Publisher left_image_pub_2;
image_transport::Publisher right_image_pub_2;
ros::Publisher sensor_pub_2;
StereoCamera* zed_2;

void image_callback(const ros::TimerEvent& timer_event)
{
    static double last_timestamp = 0;
    const sl_oc::video::Frame frame = zed_1->getLastFrame();
...
{
    const sl_oc::sensors::data::Imu imuData = zed_1->getLastIMUData();
...
void image_callback_2(const ros::TimerEvent& timer_event)
{
    static double last_timestamp = 0;
    const sl_oc::video::Frame frame = zed_2->getLastFrame();
    if (frame.data != nullptr && frame.timestamp != last_timestamp)
    {
        ...
        cv_left_image.header.frame_id = "left_frame";
        cv_left_image.header.stamp = ros::Time(last_timestamp * 1e-9);
        left_image_pub_2.publish(cv_left_image.toImageMsg());
        ...
        cv_right_image.header.frame_id = "right_frame";
        cv_right_image.header.stamp = ros::Time(last_timestamp * 1e-9);
        right_image_pub_2.publish(cv_right_image.toImageMsg());
    }
}

void sensor_callback_2(const ros::TimerEvent& timer_event)
{
    const sl_oc::sensors::data::Imu imuData = zed_2->getLastIMUData();
    if (imuData.valid == sl_oc::sensors::data::Imu::NEW_VAL) // Uncomment to use only data syncronized with the video frames
    {
        sensor_msgs::Imu imu;
        imu.header.stamp = ros::Time(imuData.timestamp * 1e-9);
...
{
    ros::init(argc, argv, "zed_node_2_cameras");
    ros::NodeHandle nh;
    ...
    left_image_pub_2 = it.advertise("left/image_raw_2", 10);
    right_image_pub_2 = it.advertise("right/image_raw_2", 10);
    sensor_pub_2 = nh.advertise<sensor_msgs::Imu>("imu/raw_2", 100);

    ros::Timer first_image_timer;
    ros::Timer first_sensor_timer;
    ros::Timer second_image_timer;
    ros::Timer second_sensor_timer;

    // get ros param
    int resolution;
    private_nh.param("resolution", resolution, 1);
    correctFramerate(resolution);
    //zed_1 = new StereoCamera(gResolution, gFps);
    //posielam rovnake rozlisenie, ak nebude rovnake, tak prerobit na gResolution2 atd.
    //zed_2 = new StereoCamera(gResolution, gFps);
    std::cout << "WHERE DA SECOND CAMERAAAAAAAAAAAAAAAAA." << endl;
    std::cout << zed_2 << endl;
    if (zed_1->camera)
    {
        first_image_timer = nh.createTimer(ros::Duration(0.01), image_callback);
    }/*
    if (zed_1->sensor)
    {
        first_sensor_timer = nh.createTimer(ros::Duration(0.001), sensor_callback);
    }*/
    if (zed_2->camera)
    {
        second_image_timer = nh.createTimer(ros::Duration(0.01), image_callback_2);
    }
    if (zed_2->sensor)
    {
        second_sensor_timer = nh.createTimer(ros::Duration(0.001), sensor_callback_2);
    }
    if (!zed_1->camera)
    {
        ROS_ERROR("Failed to initialize the first camera");
        return EXIT_FAILURE;
    }
    if (!zed_2->camera)
    {
        ROS_ERROR("Failed to initialize the second camera");
        return EXIT_FAILURE;
    }
    ros::spin();
}
